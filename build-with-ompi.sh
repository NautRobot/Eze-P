...
###############################################################################
# This warning is here to preface that this script instead
# uses rocm 6.2.4, since that was the version openmpi was 
# built with. If OpenMPI is rebuilt with rocm 7.2.0, it 
# can be used (I believe)
###############################################################################
...
# Custom paths for OpenMPI and Libfabric matching your system layout
...
if module -t list 2>&1 | grep -q '^darshan-runtime/'; then
    echo "ERROR: darshan-runtime is still loaded and could contaminate MPI detection."
    exit 1
fi
...
for required_command in cmake hipcc hipconfig mpicxx ldd; do
...
# CONFIGURE ROCSHMEM
#
# RO  = cross-node communication through Libfabric/OFI
...
cmake \
    ...
    -DROCSHMEM_NET_BACKEND=OFI \
    -DEXTERNAL_LIBFABRIC_DIR="$OFI_DIR" \
    -DPROFILE=OFF \
    -DUSE_GDA=OFF \
    -DGDA_MLX5=OFF \
    -DGDA_BNXT=OFF \
...
echo "  RO:  enabled for cross-node communication (OFI/Libfabric Backend)"
